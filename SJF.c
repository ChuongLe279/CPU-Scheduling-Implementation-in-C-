...
int main(){
    ...
    while (iTerminated < iNumberOfProcess) {
        ...
        //Xử lý các process trong ReadyQueue
        if (iReady > 0) {
            //Chỉnh sửa ReadyQueue[0] thành ReadyQueue có job ngắn nhất
            //PCB shortestProcess[10]; 
            int index = CheckShortestJob(iReady, ReadyQueue);
            ReadyQueue[index].iStart    = Max(ReadyQueue[index].iArrival , currentTime);            
            ...
            currentTime = ReadyQueue[index].iFinish;
