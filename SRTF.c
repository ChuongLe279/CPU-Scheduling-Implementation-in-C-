...
int main(){
    ...
    while (iTerminated < iNumberOfProcess){
        ...
        //Chọn ra tiến trình nào có thời gian chạy còn lại nhỏ nhất để chạy tiếp
        int indexMin = CheckShortestJob(iReady, ReadyQueue);
        PCB* Running = &ReadyQueue[indexMin];
        pushProcess(&iGantt, GanntChart, ReadyQueue[indexMin]);
        ...
        int runTime;
        int SupposedFinishTime = currentTime + Running->iBurst;

        int nextProcess;
        if (iRemain > 0){
            nextProcess = Input[0].iArrival;
        }else{
            nextProcess = INT_MAX;
        }

        if (nextProcess < SupposedFinishTime){
            runTime = nextProcess - currentTime;
        }else{
            runTime = Running->iBurst;
        }

        Running->iBurst -= runTime; //Thời gian chạy còn lại
        currentTime += runTime;
        ...
        if (Running->iBurst == 0){
            ...
            removeProcess(&iReady, indexMin, ReadyQueue);
        }
    }
